...
export class OpenAISdkAIClient implements AIClient {
    ...
    async consult(questionId: Question['id'], question: string, response: string, exampleResponses: string[]): Promise<AssistanceResponse> {
        try {
            const message = this.getInitialMessage(question, response, exampleResponses);
            ...
            return {
                ...JSON.parse(assistantMessage.replace('```json', '').replace('```', '')),
                question,
                questionId,
                response,
            };
    ...
    private getInitialMessage(question: string, response: string, exampleResponses: string[]): ChatCompletionSystemMessageParam {
    ...
    private getExamplesPrompt (exampleResponses: string[]): string {
        ...
        let examplesPrompt = 'Use these Strong yes examples to help the AI understand how a strong yes response looks like:\n';
