...
class ConsultByAudio {
    constructor(private readonly sttClient: STTClient, private readonly clientAssistanceService: ClientAssistanceService) { }

    async invoke(id: Question['id'], question: string, audioPath: string): Promise<AssistanceResponse> {
        ...
        const response = await this.clientAssistanceService.consultByText(id, question, content);
...
export const consultByAudio = new ConsultByAudio(sttClient, clientAssistanceService);